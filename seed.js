...
async function main() {
  ...
  // Default Course 1
  const existingCourse1 = await prisma.course.findFirst({ where: { title: 'Advanced React Architecture' } });
  if (!existingCourse1) {
  ...
  // Default Course 2
  const existingCourse2 = await prisma.course.findFirst({ where: { title: 'Mastering Cyber Security' } });
  if (!existingCourse2) {
    await prisma.course.create({
      data: {
